...
public:
    ...
    //long long findItem(T) const;
    void clear();

    //friend std::ifstream& operator>>(std::ifstream&, Vector<T>&);
    //friend std::ofstream& operator<<(std::ofstream&, const Vector<T>&);

    void readFromFileObj(std::ifstream&);
    void writeInFileObj(std::ofstream &)const;
    void readFromFilePrim(std::ifstream&);
    void writeInFilePrim(std::ofstream &)const;
};
...
template <typename T>
void Vector<T>::readFromFileObj(std::ifstream& inFile) {
...
template <typename T>
void Vector<T>::writeInFileObj(std::ofstream &outFile) const{
    outFile.write((const char*)&size, sizeof(size));
    outFile.write((const char*)&capacity, sizeof(capacity));
    for (int i = 0; i < size; ++i) {
        data[i] >> outFile;
    }
}
...
template <typename T>
void Vector<T>::readFromFilePrim(std::ifstream& inFile) {
    inFile.read((char*)&size, sizeof(size));
    inFile.read((char*)&capacity, sizeof(capacity));
    data = new T[capacity];
    for (int i = 0; i < size; ++i) {
        inFile.read((char*)&data[i], sizeof(data[i]));
    }
}

template <typename T>
void Vector<T>::writeInFilePrim(std::ofstream &outFile) const{
    outFile.write((const char*)&size, sizeof(size));
    outFile.write((const char*)&capacity, sizeof(capacity));
    for (int i = 0; i < size; ++i) {
